...
function renderGrades(root, exams) {
  ...
  root.innerHTML = `<header class="page-heading"><div><span class="eyebrow">Academic progress</span><h1>My Grades</h1><p>Track your exam scores and progress across all classes.</p></div><div class="heading-badge">${percentage === null ? '—' : `${percentage}%`}<small>overall</small></div></header><section class="summary-grid"><div class="summary-card"><span>Exams recorded</span><strong>${graded.length}</strong></div><div class="summary-card"><span>Pending results</span><strong>${exams.length - graded.length}</strong></div><div class="summary-card accent"><span>Overall average</span><strong>${percentage === null ? '—' : `${percentage}%`}</strong></div></section><section class="page-card"><div class="section-title"><div><span class="eyebrow">Results</span><h2>Exam performance</h2></div></div>${exams.length ? `<div class="data-list">${exams.map(exam => `<article class="data-row"><div><strong>${escapeHtml(exam.title)}</strong><span>${escapeHtml(exam.class_name)} · ${formatDate(exam.exam_date)}</span></div><b class="score-pill ${exam.score === null ? 'pending' : ''}">${exam.score === null ? 'Pending' : `${exam.score} / ${exam.max_marks}`}</b></article>`).join('')}</div>` : emptyMessage('No grades yet', 'Your exam results will appear here once your mentor records them.')}</section>`;
}
...
function setupProfilePhotoUpload() {
  const form = document.getElementById('student-photo-form'); const input = document.getElementById('student-photo'); const preview = document.getElementById('student-photo-preview'); const placeholder = document.getElementById('student-photo-placeholder'); const status = document.getElementById('student-photo-status');
  input.addEventListener('change', () => { const file = input.files?.[0]; if (!file) return; if (file.size > 2 * 1024 * 1024) { status.textContent = 'Choose an image no larger than 2 MB.'; input.value = ''; return; } const reader = new FileReader(); reader.onload = () => { preview.src = reader.result; preview.hidden = false; placeholder.hidden = true; }; reader.readAsDataURL(file); });
  form.addEventListener('submit', async event => { event.preventDefault(); const file = input.files?.[0]; if (!file) return; status.textContent = 'Uploading…'; const body = new FormData(); body.append('photo', file); const response = await fetch('/api/student/profile/photo', {method: 'POST', body}); const data = await response.json(); status.textContent = response.ok ? 'Profile photo updated.' : (data.error || 'Could not upload photo.'); status.className = response.ok ? 'success-text' : 'error-text'; });
}

async function saveSettings(event) { event.preventDefault(); const form = event.currentTarget; const status = document.getElementById('settings-status'); status.textContent = 'Saving…'; const response = await fetch('/api/student/profile', {method: 'PATCH', headers: {'Content-Type': 'application/json'}, body: JSON.stringify(Object.fromEntries(new FormData(form)))}); const data = await response.json(); status.textContent = response.ok ? 'Saved successfully.' : (data.error || 'Could not save changes.'); status.className = response.ok ? 'success-text' : 'error-text'; }
